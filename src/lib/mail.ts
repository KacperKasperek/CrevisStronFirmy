...
export async function sendContactEmails(data: { name: string; email: string; message: string }) {
  const from = process.env.SMTP_USER ?? "kontakt@crevis.pl";
  const to = process.env.CONTACT_TO ?? "kontakt@crevis.pl";
...
export async function sendResetEmail(email: string, url: string) {
  await transporter().sendMail({ from: `Crevis <${process.env.SMTP_USER ?? "kontakt@crevis.pl"}>`, to: email, subject: "Reset hasła do panelu Crevis", text: `Aby ustawić nowe hasło, otwórz ten link:\n${url}\n\nJeśli to nie Ty, zignoruj wiadomość.` });
}
